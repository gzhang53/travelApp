...
app.get('/weatherbit', async(req, res)=>{
    ...
    const weatherbitURL = `https://api.weatherbit.io/v2.0/forecast/daily?lat=${lat}&lon=${lon}&key=4840b6c226784ef4a3cfff214b7c7c17`;
    ...
    try{
        ...
        const day = projectData[projectData.length-1].daysRemain;

        //get the temperature from the corresponding day
        projectData[projectData.length-1].temp = weatherData.data[day-1].temp;
...
app.get('/pixabay', async(req,res)=>{
    ...
    const pixabayURL = `https://pixabay.com/api/?key=17986335-f2af7fcafd6a3f2fb5e0660e3&q=${cityname}&category=places&image_type=photo`;
    try{
